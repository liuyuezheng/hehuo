...
//        $uid = request()->param('uid');
        $room_id=request()->param('room_id');
        $count=BeescrmPizzaRoom::where('room_id',$room_id)->find();
        BeescrmPizzaRoom::where('room_id',$room_id)->update(['p_count'=>$count['p_count']+1]);
...
//            $authCode =request()->param('code');
    ...
    public function test(){
        $tid=BeescrmAppPizzaQuestion::where('type',0)->select();
//        $tmp = range(14,$tid);
        $ran=array_rand($tid,5);
        return $this->success('开启成功，进入答题模式','',$ran);
//        $str=$tmp[$ran[0]];
//        for($i=1;$i<$num;$i++){
//            $str=$str.",".$tmp[$ran[$i]];
