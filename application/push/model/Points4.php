        ...
        if ($type==1){
            ...
            if($num==1){
                ...
                if(in_array($uid,$uids)){
                    ...
                    $data['users'][0]=$arr[1];
                    $data['users'][1]=$arr[0];
                    $data['users'][0]['user']=$user2;
                    $data['users'][1]['user']=$user;
                }
...
//    }
    ...
    public function updatePoint($uid,$room_id,$point,$pktime,$num,$type){
        if($type==1){
            ...
            $res=self::where('room_id',$room_id)->select();
            foreach ($res as $k=>$v){
                if(in_array($uid,explode(",",$v['user_id']))){
    ...
    public function calculate($arr,$type){
        ...
        if($arr[0]['point']>$arr[1]['point']){
            foreach ($arr[0]['user'] as $k=>$v){
                ...
                User::where('user_id',$v['user_id'])->update(['count'=>$count,'points'=>$points]);
//                $data=$user->save(['points'=>$points,'count'=>$count],['user_id'=>$v['user_id']]);
            }
            foreach ($arr[1]['user'] as $key=>$val){
                if($type==1){
                    if($val['count']>0){
                        $count2=$val['count']-1;
                    }else{
                ...
                User::where('user_id',$val['user_id'])->update(['count'=>$count2]);
